pipeline{
   ...
   // }        
        ...
        steps {
        sshagent(['zap']) {
         sh 'ssh -o  StrictHostKeyChecking=no ec2-user@13.233.105.243 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://65.0.71.99:8080/dockeransible/" || true'
        }
      }
    }
    }
