pipeline{
   ...
   // }        
        ...
        steps {
        sshagent(['zap']) {
         sh 'ssh -o  StrictHostKeyChecking=no ec2-user@3.110.104.252 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://65.0.6.100:8080/dockeransible/" || true'
        }
      }
    }
    }
