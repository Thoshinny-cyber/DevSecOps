pipeline{
   ...
   // }        
        ...
        steps {
        sshagent(['zap']) {
         sh 'ssh -o  StrictHostKeyChecking=no ec2-user@3.111.32.96 "docker run -t -v /var/lib/jenkins/workspace/DevSecOps_Pipeline:/zap/wrk --user 995:993 -e ZAP_CONFIG="zap.yaml" owasp/zap2docker-stable zap-baseline.py -t http://3.109.202.193:8080/dockeransible/ -r report.html" || true'
        }
      }
    }
    }
