pipeline{
   ...
   // }        
        ...
        steps {
        sshagent(['zap']) {
         sh 'ssh -o  StrictHostKeyChecking=no ec2-user@65.0.19.96 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://13.232.102.74:8080/dockeransible/" || true'
        }
      }
    }
    }
